...
class FoodObj:
    def __init__(self, coordinates, t_y=1, agent1_reward=1, agent2_reward=2):
...
class GameEnv:
    ...
    def reset(self):
        ...
        self.agent1_actions = [self.agent1.move_forward, self.agent1.move_backward, self.agent1.move_left,
                               ...
                               self.agent1.stay]  # self.agent1.turn_left, self.agent1.turn_right, self.agent1.beam,
        self.agent2_actions = [self.agent2.move_forward, self.agent2.move_backward, self.agent2.move_left,
                               self.agent2.move_right,
                               self.agent2.stay]  # self.agent2.turn_left, self.agent2.turn_right, self.agent2.beam
        self.agent1_beam_set = []
        self.agent2_beam_set = []
        ...
        for y in range(0, 3):
            ...
            for x in range(0, 16):
                ...
                agent1_reward = self.positive_reward if is_apple else self.negative_reward
                agent2_reward = self.positive_reward if is_apple else self.negative_reward
                self.food_objects.append(FoodObj((x, y), t_y=food_type, agent1_reward=agent1_reward,
    ...
    def move(self, agent1_action, agent2_action):
        assert agent1_action in range(5), 'agent1 take wrong action'
        assert agent2_action in range(5), 'agent2 take wrong action'
        ...
        a1_action = self.agent1_actions[agent1_action]
        agent1_action_return = a1_action(env_x_size=self.size_x, env_y_size=self.size_y)
        self.agent1_beam_set = [] if agent1_action != 6 else agent1_action_return

        a2_action = self.agent2_actions[agent2_action]
        agent2_action_return = a2_action(env_x_size=self.size_x, env_y_size=self.size_y)
    ...
    def state_matrix(self):
        # 0=nothing, 1=player1, 2=player2, 3=lemon, 4=apple
        a = np.zeros([self.size_y, self.size_x])
        for food in self.food_objects:
            if food.type == 1:  # apple
                a[food.y, food.x] = 4
            elif food.type == 3:
        ...
        a[self.agent1.y, self.agent1.x] = 1
        a[self.agent2.y, self.agent2.y] = 2
        return a
