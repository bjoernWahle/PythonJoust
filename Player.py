...
class Player(pygame.sprite.Sprite):
    ...
    # TODO move to game
    def check_collisions(self, other_players, enemies, platforms, egg_list):
        ...
        collided_player = pygame.sprite.spritecollide(self, other_players, False, collided=pygame.sprite.collide_mask)
        if collided_player:
            for o_player in collided_player:
                ...
                if self.alive == 2 and o_player.alive == 2:
                    if o_player.y > self.y:
                        ...
                        self.score += self.game.rewards['defect']
                        self.other_player_kill_count += 1
                        o_player.bounce(self)
                    elif o_player.y < self.y - 5:
                        self.bounce(o_player)
                        o_player.bounce(self)
                        o_player.score += self.game.rewards['defect']
    ...
    def set_won(self, value):
        if value:
            self.score += self.game.rewards['win']
        self.has_won = value